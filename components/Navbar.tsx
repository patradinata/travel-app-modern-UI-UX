...
const Navbar = () => {
  return (
    <nav className="flexBetween mx-auto max-w-[1440px] px-6 lg:px-20 relative py-6 border-b">
      <Link href="/">
      ...
      <ul className="hidden h-full gap-12 lg:flex font-bold">
        {NAV_LINKS.map((link) => (
          <Link href={link.href} key={link.key} className="text-base font-normal text-gray-500 flex items-center justify-center cursor-pointer pb-1.5 transition-all hover:text-gray-600">
            {link.label}
          </Link>
        ))}
      </ul>
